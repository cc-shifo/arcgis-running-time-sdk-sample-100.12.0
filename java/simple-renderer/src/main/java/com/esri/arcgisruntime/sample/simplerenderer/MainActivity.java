...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // create a map with the imagery basemap. This will set the map to have a WebMercator
      // spatial reference
        ...
        //This envelope is using the WGS84 points above, but is reprojected by the mapview into
      // the maps spatial
      ...
      // from the renderer)
        ...
        //create graphics from the geyser location points. NOTE: no need to set the symbol on the
      // graphic because the
        // renderer takes care of it
        //The points are in WGS84, but graphics get reprojected automatically, so they work fine
      // in a map with a spatial
        ...
        /**
         * 两种方式显示点：
         * 1.通过render方式。整个图层用一个render，之后只需往图层添加指定位置的mark。mark样式表示用simple，
         * mark位置用点的位置来表示。这个demo就是这样实现。
         *
         * 2.simple-marker-symbol例子。直接用symbol去显示。render和symbol同时存在时，simple优先。
         * 图层不添加render，在生成图形时就将具体点和symbol绑定，然后将生成的图形添加如图层。
         */
    }
